...
class BlandService {
  ...
  async createAgent(agentConfig: AgentConfig) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Failed to create agent: ${response.status} ${errorData}`
        );
      }
  ...
  async startCall(phoneNumber?: string) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `Failed to start call: ${response.status} ${errorText}`
        );
      }
      ...
      const pollInterval = setInterval(async () => {
        try {
          const response = await fetch(
            `${BLAND_API_URL}/v1/calls/${this.callId}`,
            {
              headers: {
                authorization: this.config!.apiKey,
              },
  ...
  async sendMessage(message: string) {
    ...
    try {
      ...
      const response = await fetch(
        `${BLAND_API_URL}/v1/calls/${this.callId}/send`,
        {
          method: "POST",
          headers: {
            authorization: this.config.apiKey,
